...
function createOffsetClones(target, numberOfClones){
    ...
    for(let i=0;i<numberOfClones;i++){
      ...
      clone.style.backgroundColor = palette[i%palette.length];
      
      clone.style.transition = '1s ease all';
      clone.innerText = 'Show the clones!';


      

      createdElements.push(clone);
...
function click(event){
	...
	for (let i=0;i<elems.length;i++){
		...
		elems[i].style['z-index'] = -1-i;
		elems[i].style.top = (box.offsetHeight + margin) * (i + 1);
	}
isOpen = true;
